...
TESTCASE(langmorph_letter_group_index_id_increment)
{
	langmorph::letter_group_id id{};
...
TESTCASE(langmorph_letter_group_index_insert_and_get)
{
	std::deque<langmorph::letter_group> letter_groups;

	letter_groups.push_back(langmorph::letter_group{"sch"});
	letter_groups.push_back(langmorph::letter_group{"s"});
	letter_groups.push_back(langmorph::letter_group{"c"});
	letter_groups.push_back(langmorph::letter_group{"h"});
	letter_groups.push_back(langmorph::letter_group{"e"});
	letter_groups.push_back(langmorph::letter_group{"l"});
	letter_groups.push_back(langmorph::letter_group{"l"});
	letter_groups.push_back(langmorph::letter_group{"a"});
	letter_groups.push_back(langmorph::letter_group{"g"});
	letter_groups.push_back(langmorph::letter_group{"r"});

	std::ranges::sort(letter_groups, letter_group_cmp{});

	auto letter_groups_in = std::queue{letter_groups};

	using storage = langmorph::letter_group_index;
	...
	std::ranges::for_each(std::begin(letter_groups), std::end(letter_groups), [current_id = langmorph::letter_group_id{1},
		&letter_groups_out,
		prev = std::string_view{},
		expected_id = langmorph::letter_group_id{}
